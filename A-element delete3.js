...
result = (function(value){
	return testArr.map((cv, index) => {
		if(value === value && cv === value){
			return index;
		}else if(value !== value && cv !== cv ){
			return index;
		}
	}).filter(cv => cv)
})(undefined)

result.reverse().forEach(cv => {
	testArr.splice(cv,1);
})
...
(function(value){
	...
	for(let i = 0; i < len; i++){
		if(value === value && testArr[0] !== value){
			testArr.push(testArr[0])
		}else if(value !== value && testArr[0] !== testArr[0]){
			testArr.shift();
			continue;
		}else if(value !== value && testArr[0] !== value){
			testArr.push(testArr[0])
		}
		testArr.shift();
	}
...
// way3: will change the origin arr
testArr = (function(value){
	return Array.from(new Set(testArr)).splice(testArr.indexOf(value), 1);
})(NaN)
...
(function(value){
	...
	for (var i = 0; i < testArr.length; i++) {
		if (testArr[i] !== value && num != i) {
			testArr[++num] = testArr[i];
		}
	}
	testArr.length = ++num;
})(null)
