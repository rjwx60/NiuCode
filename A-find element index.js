// Title: 
// 在数组 arr 中，查找与 item 相等的元素的出现的所有索引值


var testArr = [NaN,null,undefined,Infinity,{},{},NaN,null,1,1,'a','a'],
	result = [];

// way 1:
result = (function(value){
	return testArr.reduce((ac,cv,index) => {
		return ac[0] === ac[0] ? ac[0] === cv ? ac.push(index) && ac : ac : ac[0] !== cv && cv !== cv ? ac.push(index) && ac : ac;
	},[value]).slice(1)
})(NaN)


// way2: can't handle NaN
result = (function(value){
	return testArr.map((item,index) => value === item && index ).filter(cv => cv);
})(undefined)


// way3: like way2, but filter unsame map
;(function(value){
	testArr.filter((item,index) => (value === item) && result.push(index));
})(undefined);
